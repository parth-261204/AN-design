...
import { plantersImages } from "../data/plantersImages.js";

const planterCataloguePdf = "/planter-catalogue.pdf";

export default function PlantersPdfView() {
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  const currentImage = plantersImages[currentImageIndex];
  const slideCounterText = useMemo(
    () =>
      `${String(currentImageIndex + 1).padStart(2, "0")} / ${String(plantersImages.length).padStart(2, "0")}`,
    [currentImageIndex],
  );

  const goToPrevious = () => {
    setCurrentImageIndex((prev) => (prev === 0 ? plantersImages.length - 1 : prev - 1));
  };

  const goToNext = () => {
    setCurrentImageIndex((prev) => (prev === plantersImages.length - 1 ? 0 : prev + 1));
  };
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-base text-[#4f4637]">
              Slideshow of all extracted Planters catalogue images. The source file is available separately.
            </p>
          </div>
        ...
        <section className="rounded-2xl border border-[#e2d8c6] bg-white p-4 md:p-5">
          ...
          <div className="overflow-hidden rounded-xl border border-[#eadfcd] bg-[#fbf8f1] p-3">
            {currentImage ? (
              <img
                src={currentImage}
                alt={`Planters slide ${currentImageIndex + 1}`}
        ...
        <section className="mt-10">
          <div className="mb-4 flex items-center justify-between">
            ...
            <p className="text-xs text-[#6f624e]">{plantersImages.length} images</p>
          </div>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
            {plantersImages.map((src, index) => (
              <button
                key={src}
                type="button"
              ...
              >
                <img
                  src={src}
                  alt={`Planters thumbnail ${index + 1}`}
